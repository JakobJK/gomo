...
func draw_preview(im: ImmediateMesh, hem: HalfEdgeMesh) -> void:
	...
	for a in 3:
		...
		_draw_box_lines(im, _gizmo_centroid + dir * shaft, box_half, col)
	_draw_box_lines(im, _gizmo_centroid, box_half * 1.4, center_col)
	im.surface_end()
...
func _draw_box_lines(im: ImmediateMesh, center: Vector3, half: float, col: Color) -> void:
	for sz in [-half, half]:
		var c0 := center + Vector3(-half, -half, sz)
		var c1 := center + Vector3( half, -half, sz)
		var c2 := center + Vector3( half,  half, sz)
		var c3 := center + Vector3(-half,  half, sz)
		im.surface_set_color(col); im.surface_add_vertex(c0); im.surface_set_color(col); im.surface_add_vertex(c1)
		im.surface_set_color(col); im.surface_add_vertex(c1); im.surface_set_color(col); im.surface_add_vertex(c2)
		im.surface_set_color(col); im.surface_add_vertex(c2); im.surface_set_color(col); im.surface_add_vertex(c3)
		im.surface_set_color(col); im.surface_add_vertex(c3); im.surface_set_color(col); im.surface_add_vertex(c0)
	for sx in [-half, half]:
		for sy in [-half, half]:
			im.surface_set_color(col); im.surface_add_vertex(center + Vector3(sx, sy, -half))
			im.surface_set_color(col); im.surface_add_vertex(center + Vector3(sx, sy,  half))
...
func _begin_drag(mouse_pos: Vector2, camera: Camera3D) -> void:
	...
	if _active_part == _AXIS_UNIFORM:
		world_n = (-camera.global_basis.z).normalized()
	else:
		...
		world_n = (object_transform.basis * helper.normalized()).normalized()
	...
	_drag_start_proj = lh.length() if _active_part == _AXIS_UNIFORM else lh.dot(_AXIS_VECS[_active_part])
...
func _continue_drag(mouse_pos: Vector2, hem: HalfEdgeMesh, camera: Camera3D) -> void:
	...
	var factor: float
	if _active_part == _AXIS_UNIFORM:
		factor = maxf(lh.length() / _drag_start_proj, 0.001)
