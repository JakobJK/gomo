...
func _on_object_changed(obj: Node3D) -> void:
	...
	item = item.get_first_child()
	while item != null:
		if item.has_meta("node") and item.get_meta("node") == obj:
			item.set_text(0, _label(obj))
			return
		item = item.get_next()

func _label(node: Node) -> String:
	if node is Camera3D: return "Camera  " + node.name
	if node is GomoMesh: return "Geometry  " + node.name
	return node.name
