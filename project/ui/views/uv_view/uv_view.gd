...
func set_mesh(gomo: GomoMesh) -> void:
	...
	if gomo != null:
		...
		_canvas.edges         = PackedVector2Array()
		_canvas.seam_edges    = PackedVector2Array()
		_canvas.face_polygons = []
		_canvas.set_uv_vert_data([PackedInt32Array(), PackedVector2Array()])
	_canvas.reset_view()

func _refresh_canvas_uv_data() -> void:
	var hem: HalfEdgeMesh = _canvas.gomo_mesh.hem
	_canvas.edges         = hem.get_uv_edges()
	_canvas.seam_edges    = hem.get_uv_seam_edges()
	_canvas.face_polygons = hem.get_uv_face_polygons()
	_canvas.set_uv_vert_data(hem.get_uv_verts())
